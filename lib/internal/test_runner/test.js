...
const { FileTest } = require('internal/test_runner/runner');
const {
...
class Test extends AsyncResource {
  ...
  getFiles(files = []) {
    if (this instanceof FileTest) {
      const filePath = resolve(this.config.cwd, this.name);
      ArrayPrototypePush(files, filePath);
    }
    if (this.subtests) {
      for (let i = 0; i < this.subtests.length; i++) {
        this.subtests[i].getFiles(files);
      }
    }
    return files;
  }
